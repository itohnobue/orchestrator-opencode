...
| `executor` | Executes the assembled task (T1: task context is the briefing; T2/T3: template → RESEARCH DATA → task). Post-fix review is NOT its job — that is `postfix-reviewer`'s. No research of its own. |
| `postfix-reviewer` | Post-fix review ONLY — verdict APPROVED / NEEDS-FIX. |
...
| `adversarial-reviewer` | Falsification gate — optional VERIFY block; severity batch sizes are volume controls. Standalone. |
...
**Plan before executing (per triggered tier):** when a T1-T3 run is triggered, present the plan to the user BEFORE execution starts — the chosen tier, the research approach (who produces it), and the stages ahead. Once the run is triggered (start gate), no separate confirmation — present and proceed.
...
### Convergence (findings tasks — continue until no triggering CONFIRMED HIGH+ remains)

For T3 findings-type tasks (reviews, audits, discovery) whose deliverable feeds a triage/fix pipeline, the review loop continues until the verified grid has no triggering (un-folded) HIGH+ findings:

- **Trigger (mechanical — the ONLY way an additional iteration runs):** the prior VERIFY synthesis grid contains at least one triggering CONFIRMED finding at HIGH or CRITICAL severity (adversarially verified). REJECTED findings never trigger. WEAKENED findings trigger only when the corrected severity remains HIGH+. LOW findings are dropped and never trigger. The trigger is never a judgment call — no iteration fires without a triggering CONFIRMED HIGH+ in the prior grid. A CONFIRMED HIGH+ that extraction has explicitly labelled EXTENDS/CORROBORATES against an already-confirmed HIGH+ (same root cause) does not fire an iteration; anything not so labelled does (conservative default).
...
- `executor` — the ONE executor for all work (implementation, execution, deep analysis, investigation).
...
**Task file contents:** PROJECT, YOUR TASK (KEY FILES, CONTEXT, SCOPE), MUST ANSWER questions, DELIVERABLES paths (unique per agent run). **Definition of done & scope (MANDATORY):** the brief states the definition of done (the observable condition(s) that make the run complete) and what is explicitly out of scope; `MUST ANSWER` carries one item per promised deliverable/criterion — capped, never a restatement of the whole task. These ride in the existing `SCOPE` / `MUST ANSWER` fields — no separate artifact; the review's completeness sweep, the adversarial scope judgement, the fix trace, and the Completion Sanity Check all read the same list. Code tasks get a WRITABLE FILES section listing exactly which source files may be modified. **PRIOR CONTEXT quality matters:** state the module's contracts, specs, environment facts, and expected behaviors explicitly — the executor leans on them; do not expect the research phase to supply what the task file should state.
...
2. **Verifiable done** — is the deliverable checkable by a fresh reviewer — explicit outputs, evidence-backed MUST ANSWER, plus an explicit definition of done and out-of-scope in the brief (see Task file contents)?
...
The workflow applies to any project, any source of findings (log analysis, code review, user reports, test failures, audits), and any issue type — bugs, performance problems, security issues, refactors, architectural changes, or any work where each finding needs review → falsification → fix → re-verify. The chain starts at PREPARE — a prepare-agent spawn (digest + full report, FOCUS per issue) is MANDATORY for every T3 issue, no exceptions; findings of the same issue share its research, a finding on a disjoint tech area gets its own. Then REVIEW — one review agent per finding. Findings may pre-exist (for log-derived findings they are produced first by the `/process-report-logs` skill (Phase 1-2: analysis agents per log group + synthesis agent, Phase 3: report-back); other sources supply them directly) — or the review discovers them itself. **One agent per finding at every delegated stage; parallelize only across independent findings** (respect the parallel-spawn rules above; batching is OK under the general per-agent volume cap — see Task splitting above).
...
2. **ADVERSARIAL** (one adversarial reviewer per finding — each finding is its own issue, its own VERIFY block) — `adversarial-reviewer` — falsifies the review's claims: the root-cause attribution AND the fix proposal (would the fix actually work? does the code already handle the case through another path? does the proposal miss a variant? is it minimal and safe?). Verdicts CONFIRMED / REJECTED / WEAKENED per claim. REJECTED findings are dropped; WEAKENED findings are downgraded to the corrected severity and fixed at lower priority.
...
6. **Explain, don't dump** — operator replies carry explanations, not code. No code
   blocks or raw line quotes unless the operator asked for them or the quoted line is
   itself the subject of the answer.
...
- **Plain, honest, evidenced.** Short sentences, common words; explain jargon
  ...
  Verification is internal: cite a line or paste output only when the operator
  asks, or when that exact line/output is the subject of the answer; otherwise
  state the finding in prose.
