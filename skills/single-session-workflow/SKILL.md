...
**Research when the answer depends on it.** Whenever the answer or the work depends on external facts — versions, APIs, documentation, news, best practices, unfamiliar technologies — and you are not confident in your knowledge, use `web_search.sh` instead of answering from memory. Guessing verifiable facts is the failure mode; searching when unsure is the fix. If the fact is stable, well-known, or already in the task file, memory suffices — no search needed.
...
**Mechanics — when you do search, use `web_search.sh`**: no built-in websearch tool, no WebFetch tool, no `curl` against APIs, no manual GitHub API calls, no `wget`, nothing else. Use `./.opencode/tools/web_search.sh "query"` (or `.opencode/tools/web_search.bat` on Windows):
...
- **FULL OUTPUT — agents must never trim results** — `prepare-agent` and `web-searcher` (and any agent running `web_search.sh`) must never pipe the tool through trimming utilities (`tail`, `head`, `less`, `more`, `grep -m`): search results are the research raw material, and trimmed output loses sources. If the tool reports truncation, read the full saved output file it points to.
