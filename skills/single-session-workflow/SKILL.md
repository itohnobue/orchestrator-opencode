...
- **Subagent reuse (resume same session via `task_id`)** — reuse is ALWAYS the lead's call; the envelope below defines what it is and the boundaries against regressions, not when it must be used:
  ...
  - **R3 Threshold (not hard cap) — watch at 3 resumes per `task_id`** (recovery resumes in Error Handling count too): each resume replays the full transcript — monotonic cost; past ~3, quality/token regression is likely, so write a handoff summary to the report path and boot a fresh successor (new name, new run, reads the file). The lead may retire earlier. Reuse ≠ respawn — separate from the `-r2/-r3` budget.
