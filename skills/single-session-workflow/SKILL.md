...
Big delegated tasks split into multiple agent runs BEFORE tiering, per mechanical limits (empirically validated in orchestrator runs). The main model applies these rules itself — they are counting, not judgment:
...
Split sub-runs keep their own task files and report paths; each is a separate delegation run (own tier per the context rule, own second opinion if applicable).
