...
5. Delegate via the `task` tool — pass the file path with a read-and-execute instruction, NOT the full content:
   ...
   ```
6. **T3 — the full workflow** (any complex issue; findings tasks at MEDIUM+): the review agents run first (one per finding — research + discovery + review, see the T3 full workflow below) — their assembled output is the primary opinion. Then the delegated pipeline: the research-backed s2 on the same scope (one prepare with complementary FOCUS + one s2 executor, own paths — see Second-opinion rules), then the VERIFY block (step 7) and the fix chain, then the final KNOWLEDGE HARVESTING stage (main model, see T3 full workflow step 7).
...
3. **Decisions baked** — every genuine fork resolved *in the prompt* with reasoning, no "you decide" left to the executor?
4. **Fresh-read** — reading the prompt alone, no conversation memory: anything ambiguous or assumed?
5. **References exist** — every path the prompt points to (prior reports, research files, WRITABLE FILES) verified present?
