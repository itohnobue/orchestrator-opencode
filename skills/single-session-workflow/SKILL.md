...
**Mechanics — when you do search, use `web_search.sh`**: no built-in websearch tool, no WebFetch tool, no `curl` against APIs, no manual GitHub API calls, no `wget` for search. Fetching a specific known URL goes through `web_search.sh --url <url>` (direct fetch mode, up to 50k chars) — the sanctioned way to get a named page when a search would be wasteful. Use `./.opencode/tools/web_search.sh "query"` (or `.opencode/tools/web_search.bat` on Windows):
