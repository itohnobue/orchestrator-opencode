...
description: Switch to single-session mode. The main model stops acting as the orchestrator lead and becomes the single-session worker: it does the work itself in dialog with the user, delegating only per the research-backed executor pipeline rules below. Use when the user wants direct single-session behavior instead of the orchestration workflow.
...
8 agents for OpenCode, built around the **research-backed executor pipeline**: the context rule guides when a briefing adds value; research data (digest + full report) is a GENERAL input to any executor run — the main model decides per spawn whether to give one, and who produces it (prepare agent or its own curation). Prepare agent and research-backed second opinions supply fresh research; verification is optional. Agents are stored in `.opencode/agents/` as Markdown files with YAML frontmatter. Full directory: `.opencode/agents/INDEX.md` — read it before delegating.

| Agent | Role |
|-------|------|
| `prepare-agent` | Research generation when fresh web research is needed (any executor run). Identifies every technology a task touches, researches up to 3 queries per technology (best practices, real domain knowledge, specialist advice), curates the highest-quality material into a FULL research report (no size cap) plus a COMPACT digest (~10KB soft max) that the executor's prompt carries. `FOCUS:` parameter defines the specialist identity. Speed-limited by design. |
| `executor` | Executes the assembled task (plain: task context is the briefing; with briefing: template → RESEARCH DATA → task). Runs with high reasoning effort (default). No research of its own. |
...
## Agent Delegation (research-backed executor pipeline)
...
**When to delegate:**
1. **No 100%-fit existing agent** exists for the job (the job needs a custom specialist — a plain executor when no briefing is needed, research generation plus a dedicated executor when one is).
...
**Research treatment (per spawn — guidance, not a gate):** research data (digest + full report, see ASSEMBLE below) is a GENERAL input available to ANY executor run. The rule of thumb: **when you have something valuable and BIG to give an agent, inject it via research, using the digest + full report scheme** — the prompt carries only the compact digest, the depth lives in the report file the executor consults on demand. The patterns below guide the judgment; they are guidance, not a classification that forbids research in a "plain" run or forces it in a "researched" one:
- **Plain** — no briefing: the task file's own context is the briefing. Best when (a) self-contained tasks — logic-internal, contracts and expected behaviors stated; (b) well-researched scopes whose facts are already in the task file (main model's own research, prior sessions, project knowledge). When the main model's own research is substantial (more than a couple of KB of facts), prefer the digest + full report split below over baking it all into PRIOR CONTEXT — the task file stays lean and the briefing stays consult-on-demand. Implementations follow the rule too: plain when specs/contracts are stated, researched when they depend on current external facts the file does not carry.
- **Researched** — a briefing is injected (digest + full report): the prepare agent's fresh web research, or the main model's own curation. Best when the file is thin and the task depends on facts it does not carry (current external facts: versions, APIs, ecosystem behavior, format specs, security advisories), or when report precision/breadth matters (findings feed triage/fix pipelines, user-facing reviews).
- **Second opinion (s2)** — findings/analysis tasks: a research-backed second run with complementary FOCUS on top of a primary whose treatment follows the same guidance (plain when the task file is rich, researched when thin). The best measured combination; role-only s2 is dropped.
...
1. **DECIDE RESEARCH TREATMENT** — apply the context rule and the treatment guidance: plain (no briefing — rich context only), researched (a briefing adds value — thin context needing external facts, or precision/breadth matters, or anything valuable and big worth injecting), or second opinion (findings/analysis at MEDIUM+: primary per the guidance + research-backed s2). For plain runs skip to ASSEMBLE without `--research-file`.
2. **PREPARE** (when fresh web research is needed) — spawn `prepare-agent` with a prepare task (includes `FOCUS: <angles>`, default `correctness, completeness`). Output: `tmp/prepare/<slug>-research.md` (full report — no size cap) + `tmp/prepare/<slug>-digest.md` (soft max ~10KB, 1-2KB over fine). The prepare agent self-reviews its files before delivery (digest size + full-report coverage, confidence tiers, policy baking, source mapping, no raw dumps) — the main model does NOT check; it only acts if the prepare report flags remaining issues. Prepare does research data only: no pre-solving, no search-output trimming, knowledge fallback if web search fails.
...
4. **EXECUTE** — spawn `executor` for the work. It reads the file, does the work, reports. **The delegation run ends here.** Verification is NOT part of it — it is a separate optional block (see VERIFY below), used for critical issues, acted-on findings, or on demand.

### Task splitting (volume caps — applied by the main model)

Big delegated tasks split into multiple agent runs BEFORE research treatment, per mechanical limits (empirically validated in orchestrator runs). The main model applies these rules itself — they are counting, not judgment:
...
Split sub-runs keep their own task files and report paths; each is a separate delegation run (own research treatment per the context rule, own second opinion if applicable).
...
- **How (context rule + research-backed s2 — no role-only s2):** a second-opinion run pairs the primary — treated per the context rule (plain when the task file is rich, researched when thin) — with one prepare with a complementary FOCUS (e.g., primary `security, correctness` → second `performance, maintainability`) feeding the s2 executor. Never the same FOCUS twice — unique catches cluster in the complementary FOCUS areas. The prepare runs CONCURRENTLY with the primary executor; the s2 executor runs after assembly. The s2 recovers unique bugs the primary missed.
...
For findings-type delegated tasks (reviews, audits, discovery) whose deliverable feeds a triage/fix pipeline, the review loop continues until the verified grid is clean of HIGH+ findings:

- **Trigger (mechanical — the ONLY way an additional iteration runs):** the prior VERIFY synthesis grid contains at least one CONFIRMED finding at HIGH or CRITICAL severity (adversarially verified). REJECTED findings never trigger. WEAKENED findings trigger only when the corrected severity remains HIGH+. LOW findings are dropped and never trigger. The trigger is never a judgment call — no iteration fires without a CONFIRMED HIGH+ in the prior grid, and a CONFIRMED HIGH+ in the grid always fires one (if the user has not capped the run).
...
- **Ceiling:** the user may cap the loop (e.g., "max 2 iterations") at spawn time for time/cost control — stated in the task brief. Without a stated cap, convergence is defined by the trigger above, not by a fixed iteration count.
...
- Research treatment is chosen at ASSEMBLE time — see "Research treatment" above. A run may carry a briefing (`--research-file`/`--research-report`) or not, per the main model's judgment — all runs use the same executor.
...
**Decision order:** (1) session work → main model directly; (2) an existing agent is a 100% fit (a research question → the research agent matching its type — `web-searcher` / `research-analyst` / `data-researcher`; checking a claim → `adversarial-reviewer-max` for CRITICAL/HIGH, `adversarial-reviewer-high` for MEDIUM) → single direct agent call, no prepare+execute — but only when the fit is really 100%, never as a default reflex; (3) otherwise → full delegated run with research treatment per the context rule: plain when the task file already carries rich context; a briefing (prepare-agent research or main-model curation, digest + full report) when the file is thin — current external facts missing from it (or precision/breadth matters, or anything valuable and big worth injecting); research-backed s2 for second-opinion runs (see Research treatment under Agent Delegation). Research tasks default to the matching research agent; they are delegated only when the research task itself is big/context-hungry beyond a single research-agent run.
...
2. **Plain run (rich context only — implementations included when specs/contracts are stated):** assemble WITHOUT research:
   ```bash
...
5. Delegate via the `task` tool — pass the file path with a read-and-execute instruction, NOT the full content:
   ...
   ```
6. MEDIUM+ severity findings tasks: run the second-opinion flow — primary per the context rule (already done in step 5) + research-backed s2 (one prepare with complementary FOCUS + one s2 executor, own paths — see Second-opinion rules).
...
6. **POST-FIX ADVERSARIAL** (ONLY if any post-fix review produced MEDIUM+ findings) — ONE severity-routed adversarial reviewer per issue (`adversarial-reviewer-max`/`adversarial-reviewer-high` per the findings' severities — a single run on that issue's post-fix findings). All clean → skip.
...
**Research rule in this flow:** review and fix briefs are **plain** — the finding evidence, root-cause context, and fix design travel in the task file, which IS the briefing; the agent reads internal codebase facts itself. Research (prepare) enters only when the fix depends on current external facts the task file does not carry. **State the treatment explicitly in every brief** (e.g. "TREATMENT: plain — task file carries rich context; all facts internal"). The synthesis/report stage that produced the findings is where external research (if any) would have been baked in already.
