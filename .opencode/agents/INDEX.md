...
| File | Agent | Role |
|------|-------|------|
| executor.md | Executor | The ONE generic executor: DISCOVER, IMPLEMENT, REVIEW, FIX, TEST, TEST-UPDATE, quick-fix, build-gate, final gate, single-session tasks. Post-fix review is NOT its job — that is postfix-reviewer's. PLAIN (task file carries the research) or researched (digest injected as RESEARCH DATA + full report path). Adopts the report's FOCUS as its standpoint. No web research of its own. |
| postfix-reviewer.md | Postfix reviewer | Post-fix review ONLY (strictly read-only): verifies applied fixes against their design (correctness, minimality, new bugs, test breakage, race conditions; verdict APPROVED / NEEDS-FIX). Never used for any other task. |
