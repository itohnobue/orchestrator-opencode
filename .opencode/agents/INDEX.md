...
| File | Agent | Role |
|------|-------|------|
| executor.md | Executor | The ONE generic executor: DISCOVER, IMPLEMENT, REVIEW, FIX, TEST, TEST-UPDATE, quick-fix, build-gate, final gate, single-session tasks. High reasoning effort (max reserved for planner/organizer/adversarial/postfix-reviewer). Post-fix review is NOT its job — that is postfix-reviewer's. PLAIN (task file carries the research) or researched (digest injected as RESEARCH DATA + full report path). Adopts the report's FOCUS as its standpoint. No web research of its own. |
