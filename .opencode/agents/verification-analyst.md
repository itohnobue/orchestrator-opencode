...
9. **Write the extraction report** with a batch assignment table: every finding ID → its adversarial batch (or direct-synthesis route), severity, and tag set. MEDIUM+ findings MUST be assigned to an adversarial batch — the lead spawns the batches exactly per this table; a finding without a batch assignment is a defect.
