...
- **Stale agent names** — referenced agent `.md` file does not exist on filesystem. Verify via `ls .opencode/agents/` (the orchestrator's 11-agent set: agentic-planner, volume-splitter, agent-organizer, verification-analyst, adversarial-reviewer, web-searcher, research-analyst, data-researcher, executor, postfix-reviewer, prepare-agent). Persona names from the old system are stale references.
