---
description: Adversarial reviewer — the single falsification quality gate, always running at MAX reasoning effort. Tries to FALSIFY findings from discovery/audit stages. Reads cited source code, searches exhaustively for counter-evidence, and labels each finding CONFIRMED/REJECTED/WEAKENED following the unified verification vocabulary. Findings surviving exhaustive adversarial falsification become ADVERSARIALLY VERIFIED. Batch sizes by severity (CRITICAL 1:1, HIGH 1:3, MEDIUM 1:10) are volume controls, not effort tiers.
mode: subagent
reasoningEffort: max
tools:
  read: true
